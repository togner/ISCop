        ...
        public override void Check(Package package)
        {
            ...
            foreach (var pipe in package.GetControlFlowObjects<MainPipe>())
            {
                ...
                foreach (IDTSComponentMetaData100 comp in mainPipe.ComponentMetaDataCollection)
                {
                    ...
                    {
                        SourceAccessMode? accessMode = null;
                        if (comp.TryGetCustomPropertyValue<SourceAccessMode?>(CustomPropertyNames.AccessMode, out accessMode))
                        {
                            if (accessMode.Value == SourceAccessMode.OpenRowSet
                                || accessMode.Value == SourceAccessMode.OpenRowSetVariable)
                            {
